...
class categoryList extends Component {
  ...
  componentDidMount() {
    ...
    // this.setState({products:[], searchItems:[]})
        ...
        dept.sub_departments.map((sub) => {
          if (sub.id === this.state.subDepartment.id) {
            sub.categories?.map((cat, index) => {
              let _cat = cat;
              if (_cat.id === this.state.category.id) {
                    ...
                    if (_product.is_scalable && !_product.size.includes("CT")) {
                      ...
                      if (
                        ...
                        _regular_retail = parseFloat(
                          _product.regular_retail / 2.20462
                        ).toFixed(2);
                      }
                      if (_measureUnit === "G") {
                        _price = parseFloat(
                        ...
                        _regular_retail = parseFloat(
                          _product.regular_retail / 453.59237
                        ).toFixed(2);
                      }
                      if (_measureUnit.includes("OZ")) {
                        ...
                        _regular_retail = parseFloat(
                          _product.regular_retail / 16
                        ).toFixed(2);
                      }
                      if (_measureUnit.includes("lb")) {
                        _price = _product.unit_retail;
                        _regular_retail = _product.regular_retail;
                      }

                      _product.size = "lb";
                      _product.unit_retail = _price;
                      _product.regular_retail = _regular_retail;
                      _product.quantity = 0.25;
                    } else {
                      _product.quantity = 1;
                    }
                    if (_product.regular_retail == null) {
                      _product.regular_retail = _product.unit_retail
                    }
    ...
    //   if (cat.id === this.state.category.id) {
    //     this.setState({catIndex:Math.round(index/3)})
    //   }
    // })
    // console.log('index', this.state.catIndex,_categories)
    this.setState({ categories: _categories });
    this.setState({ searchItems: _searchItems },()=>{
      console.log('SEARCHED ITEMS', this.state.searchItems)
    });
  ...
  pagination = () => {
    const { limit, page, searchItems } = this.state;
    ...
    for (var i = 0; i < page * limit; i++) {
      ...
      _products.push(data[i]);
    }

    // _products.sort(function(a, b) {
    //   return b.is_promotional ? 1 : -1;
    // });
    this.setState(
      {
  ...
  renderFooter = () => {
    if (!this.state.loader) {
      ...
      return <View>{util.Lumper(16)}</View>;
    }
  };
  incrementPage = () => {
    setTimeout(() => {
  ...
  //   // console.log('nextState', nextState.overlayId)
  //   // console.log('currentState', this.state.overlayId)
  //   return nextState.overlayId === this.state.overlayId
  // }
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      if (this.props.fetchedListItems) {
        console.log(this.props.fetchedListItems, "listitems fetched");
    ...
    if (
      ...
      JSON.stringify(prevProps.departments) !==
        JSON.stringify(this.props.departments)
    ) {
      _categories = [];
      _searchItems = [];
      // this.setState({products:[], searchItems:[]})
              ...
              sub.categories?.map((cat, index) => {
                ...
                if (_cat.id === this.state.category.id) {
                  ...
                  _cat.products?.map((product) => {
                    if (product) {
                      let _product = product;
                      console.log(product.size, "size,didupdate");
                      if (
                  ...
                  data.map((item) => {
                    ...
                    if (_item.is_scalable && !_item.size.includes("CT")) {
                      ...
                      if (_measureUnit.includes("OZ")) {
                        ...
                        _regular_retail = parseFloat(
                          _item.regular_retail / 16
                        ).toFixed(2);
                      }

                      if (_measureUnit.includes("lb")) {
                        _price = _item.unit_retail;
                        _regular_retail = _item.regular_retail;
                      }
                    ...
                    if (_item.regular_retail == null) {
                      _item.regular_retail = _item.unit_retail
                    }
  ...
  modalAddList() {
    return (
      ...
      >
        <View style={{ flex: 1 }}>
          <View style={{ marginTop: util.WP(20) }}>
              ...
              _searchItems.map((item, index) => {
                list.products?.map((product) => {
                  if (item.id == product.id) {
                    ...
                    if (item.is_scalable) {
                      if (item.is_scalable && !item.size.includes("CT")) {
                        ...
                        if (_measureUnit.includes("lb")) {
                          ...
                          _regular_retail = item.regular_retail;
                        }
                        item.size = "lb";
                        item.unit_retail = _price;
                        item.regular_retail = _regular_retail;
                        item.quantity = 0.25;
                      } else {
                        item.quantity = 1;
                      }
                    ...
                    if (item.regular_retail == null) {
                      item.regular_retail = item.unit_retail
                    }
                  }
                });
              });
          ...
          _searchItems.map((item) => {
            ...
            if (item.is_scalable) {
              if (item.is_scalable && !item.size.includes("CT")) {
                ...
                if (_measureUnit.includes("OZ")) {
                  _price = parseFloat(item.unit_retail / 16).toFixed(2);
                  _regular_retail = parseFloat(item.unit_retail / 16).toFixed(
                    2
                  );
                }
  ...
  goToCheckout() {
    if (this.state.fetchedListItems) {
      console.log("fetched Lists", this.state.fetchedListItems);
      let _orderProducts = [];
      if (this.state.fetchedListItems.products.length) {
        _orderProducts = this.state.fetchedListItems.products;
        console.log(this.state.fetchedListItems, "order products");
        _orderProducts.map((product, index) => {
          _orderProducts[index].stockQuantity = "0";
          if (product.is_scalable) {
            if (
              product.is_scalable &&
              !product.size.includes("CT") &&
              !product.size.includes("lb")
            ) {
              let _measureUnit = product.size;
              console.log(_measureUnit, product.size);
              let _price = null;

              if (_measureUnit.includes("KG")) {
                _price = parseFloat(product.unit_retail / 2.20462).toFixed(2);
                console.log("normal product inside if kg", product.unit_retail);
              }
              if (_measureUnit === "G") {
                _price = parseFloat(product.unit_retail / 453.59237).toFixed(2);
                console.log("normal product inside if g", product.unit_retail);
              }
              if (_measureUnit.includes("OZ")) {
                _price = parseFloat(product.unit_retail / 16).toFixed(2);
                console.log("normal product inside if oz", product.unit_retail);
              }
  ...
  FilterbyDiscount() {
    // console.log('products to filter', this.state.products)
    let stateProducts = [...this.state.products];
    ...
    // const promotionArr = stateProducts.filter((item) => item.is_promotional);
    // const discounted = stateProducts.filter(
    //   (i) => Number(i.unit_retail) < Number(i.regular_retail)
    // );
    // const discountedSorted = discounted.sort(
    //   (a, b) => Number(a.unit_retail) - Number(b.unit_retail)
    // );
    ...
    //   (a, b) => Number(a.unit_retail) - Number(b.unit_retail)
    // );
    // const remainingArr = stateProducts.filter(
    //   (item) =>
    //     !item.is_promotional &&
    //     Number(item.unit_retail) === Number(item.regular_retail)
    // );

    // const final = sortedPromotion.concat(discountedSorted, remainingArr);
    // this.setState({ products: final }, () => {
    //   console.log("products after filer", this.state.products);
    // });
    ...
    //   remainingArr = stateProducts.map((item) => {
    //     if (
    //       item.is_promotional &&
    //       Number(item.unit_retail) !== Number(item.regular_retail)
    //     ) {
    ...
    // const final = sortedPromotion.concat(remainingArr);
    // console.log(final, "final");
    // this.setState({ products: final });
  }
      ...
      return (
        ...
        >
          ...
          <View style={{ flexGrow: 1, flex: 1 }}>
            ...
            <View style={{ marginTop: util.WP(1), marginBottom: util.WP(2) }}>
              <FlatList
                horizontal={true}
                showsHorizontalScrollIndicator={false}
