...
class ShopNow extends Component {
  ...
  triggerSearch = () => {
    if (this.props.user && this.state.searchTerm.length > 2) {
      this.setState({ localSearch: true, offset: 0 });
      let _payload = {
  ...
  handleCatClick = (category, dept, sub) => {
    ...
    this.props.GetCategoryProducts(data);
    analytics().logEvent("Categories", {
      category: category.name,
      country_code: this.props.user.user_info.country,
    });
  ...
  handleSearchCatClick = (cat) => {
    console.log(cat, "hadnle search cat");
    if (cat) {
      ...
      this.props.departments?.map((dept) => {
        if (cat.department_id === dept.id) {
          console.log("insied depart");

          _department = dept;
          _department.sub_departments?.map((sub) => {
            if (sub.id === cat.sub_department_id) {
              console.log("insied sub");

              sub.categories?.map((category) => {
                console.log("insied category");
  ...
  decreseQuantity = (id, type) => {
    ...
    this.setState({ searchItems: resultArray });
  };
  ...
  orderNow() {
    ...
    // let _orderProducts = [];
    // _orderProducts = this.props.fetchedLists.find(
  ...
  pagination = () => {
    ...
    this.props.advanceSearch(data);
    this.setState({ refresh: !this.state.refresh });
  };
  renderFooter = () => {
    if (!this.props.ShopCatLoader) {
      return null;
    } else {
      return (
        <View>
...
const mapStateToProps = (state) => {
  return {
    ...
    advanceSearchItems: state.shop.advanceSearchItems,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    GetCategoryProducts: (data) => dispatch(TASKS.categoryProductsShop(data)),
  };
};
